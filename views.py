        ...
        if cliente_form.is_valid() and direccion_formset.is_valid():
            ...
            if not is_supervisor(request.user):
                try:
                    ...
                    messages.error(request, _("Tu usuario no está registrado como agente de ventas."))
...
def editar_cliente(request, pk):
    ...
    if not is_supervisor(request.user) and cliente.agente.user != request.user:
        return HttpResponseForbidden(_("No tienes permiso para editar este cliente."))
...
def confirmar_eliminar_cliente(request, pk):
    ...
    cliente = get_object_or_404(Cliente, pk=pk)
    if not is_supervisor(request.user) and cliente.agente.user != request.user:
        return HttpResponseForbidden(_("No tienes permiso para eliminar este cliente."))

    if request.method == 'POST':
        cliente.delete()
        messages.success(
            request, 
            _(f'El cliente "{cliente.nombre_razon_social}" fue eliminado correctamente.'))
        return redirect('lista_clientes')
...
def editar_direccion(request, pk):
    ...
    if not is_supervisor(request.user) and cliente.agente.user != request.user:
        return HttpResponseForbidden(_("No tienes permiso para editar esta dirección."))
...
def eliminar_direccion(request, pk):
    ...
    if not is_supervisor(request.user) and cliente.agente.user != request.user:
        ...
        return HttpResponseForbidden(_("No tienes permiso para eliminar esta dirección."))
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            error = _("Usuario o contraseña inválidos")
    else:
        error = None
...
def exportar_clientes_excel(request):
    ...
    ws = wb.active
    ws.title = _("Clientes y Direcciones")
...
def importar_clientes(request):
    ...
    if request.method == 'POST':
        ...
        if not archivo:
            form.add_error('archivo', _('Debes seleccionar un archivo antes de importar.'))
        # 2) ¿Extensión válida?
        elif not archivo.name.lower().endswith(('.xlsx', '.xls')):
            form.add_error('archivo', _('Formato no válido. Usa .xlsx o .xls.'))
        ...
        if not form.errors:
            ...
            if ImportacionLog.objects.filter(hash_archivo=hash_archivo).exists():
                form.add_error(None, _('Este archivo ya fue importado anteriormente.'))
        ...
        if form.is_valid():
            ...
            for idx, row in enumerate(sheet.iter_rows(min_row=fila_inicio), start=fila_inicio):
                ...
                if not (nombre and rut and comuna and calle):
                    fallidos += 1
                    errores.append({'fila': idx, 'error': _('Faltan campos obligatorios')})
                    continue

                if Cliente.objects.filter(rut=rut).exists():
                    fallidos += 1
                    errores.append({'fila': idx, 'error': _('Cliente ya existe')})
                    continue
            ...
            messages.success(
                request, 
                _(f"Importación finalizada: {exitosos} clientes creados, {fallidos} errores.")
            )
